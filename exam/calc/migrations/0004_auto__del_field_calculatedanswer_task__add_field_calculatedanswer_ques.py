...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'CalculatedAnswer.task'
        db.delete_column('calc_calculatedanswer', 'task_id')

        # Adding field 'CalculatedAnswer.question'
        db.add_column('calc_calculatedanswer', 'question',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=0, to=orm['calc.Question']),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'CalculatedAnswer.task'
        raise RuntimeError("Cannot reverse this migration. 'CalculatedAnswer.task' and its values cannot be restored.")
        # Deleting field 'CalculatedAnswer.question'
        db.delete_column('calc_calculatedanswer', 'question_id')
